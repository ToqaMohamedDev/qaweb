...
# أرشيف المحاولات الكامل (Timeline of Fixes)

## 1. محاولة التلاعب اليدوي بالكوكيز (Manual Cookie Copying)
- **التصرف:** تعديل `app/auth/callback/route.ts` لنسخ الكوكيز يدوياً وتعيين خصائص `SameSite=Lax`.
- **الحالة:** تم التراجع عنها جزئياً لصالح الحلول القياسية.

## 2. محاولة كسر حماية الكوكيز (Force httpOnly: false)
- **التصرف:** تعديل الـ Middleware لإجبار الكوكيز أن تكون مرئية لـ JavaScript.
- **الحالة:** **فشلت وتسبب بمشاكل أمنية** وتم إلغاؤها فوراً.

## 3. محاولة تمرير البيانات عبر Layout (Server Hydration)
- **التصرف:** جلب `user` و `profile` في `RootLayout` وتمريرها لـ `ClientProviders`.
- **الحالة:** نجحت في الـ Auth لكنها تسببت بـ **React Hydration Error #418** مما أدى لتعليق الصفحة. تم إلغاؤها.

## 4. محاولة API Mediator (الحل الجسري)
- **التصرف:** إنشاء `/api/auth/session` لقراءة الكوكيز المخفية.
- **الحالة:** **ناجحة ومفعلة حالياً**. حلت مشكلة اختفاء السيشن.

## 5. محاولة تعطيل OneSignal
- **التصرف:** إيقاف تشغيل OneSignal مؤقتاً.
- **الحالة:** إجراء تشخيصي (لم يكن هو السبب الرئيسي للتعليق لكنه كان مشتبهاً به).

## 6. محاولة Server Actions للبيانات
- **التصرف:** استبدال `supabase.from('lessons')` في المتصفح بـ `fetchHomeLessonsAction` في السيرفر.
- **الحالة:** **مفعلة حالياً**. الحل الجذري لمشاكل الشبكة والتعليق.

## 7. فك حصار الـ Redirect
- **التصرف:** تعطيل الـ Redirect التلقائي في صفحة Login.
- **الحالة:** **مفعلة حالياً** لمنع الـ Loops.
