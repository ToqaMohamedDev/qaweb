...
import type { MCQ, ComprehensiveData } from "../types";

interface ComprehensiveFormProps {
  initialData?: ComprehensiveData;
  onChange: (data: ComprehensiveData) => void;
}

export function ComprehensiveForm({ initialData, onChange }: ComprehensiveFormProps) {
  const [multipleChoiceQuestions, setMultipleChoiceQuestions] = useState<MCQ[]>(
    initialData?.multipleChoiceQuestions || []
  );
  const prevDataRef = useRef<string>(JSON.stringify(multipleChoiceQuestions));
  const isInitialMount = useRef(true);

  // Update state when initialData changes (for edit mode)
  useEffect(() => {
    if (initialData?.multipleChoiceQuestions) {
      const newData = JSON.stringify(initialData.multipleChoiceQuestions);
      const currentData = JSON.stringify(multipleChoiceQuestions);
      if (newData !== currentData) {
        setMultipleChoiceQuestions(initialData.multipleChoiceQuestions);
  ...
  useEffect(() => {
    ...
    if (prevDataRef.current !== currentData) {
      ...
      onChange({ multipleChoiceQuestions });
    }
  }, [multipleChoiceQuestions, onChange]);
