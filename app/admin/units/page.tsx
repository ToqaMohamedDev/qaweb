        ...
        <div className="space-y-8">
            ...
            <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-violet-500 via-purple-500 to-fuchsia-500 p-8">
                ...
                <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                    <div className="flex items-center gap-4">
                        ...
                        <div><h1 className="text-3xl font-bold text-white">الوحدات</h1><p className="text-white/80 mt-1">إدارة الوحدات والفروع لكل مرحلة ومادة</p></div>
                    </div>
                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => { setCurrentLesson({ is_published: false }); setIsModalOpen(true); }} className="flex items-center gap-2 px-6 py-3 rounded-xl bg-white text-purple-600 font-semibold shadow-lg shadow-black/10 hover:shadow-xl transition-all">
                        <Plus className="h-5 w-5" /><span>إضافة وحدة جديدة</span>
                    </motion.button>
                </div>
            </div>
                    ...
                    <>
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-violet-50 to-violet-100 dark:from-violet-900/20 dark:to-violet-800/20 p-5 border border-violet-200/50 dark:border-violet-700/50">
                            <div className="flex items-center justify-between">
                                <div><p className="text-sm text-gray-600 dark:text-gray-400 mb-1">إجمالي الوحدات</p><p className="text-3xl font-bold text-gray-900 dark:text-white">{lessons.length}</p></div>
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20 p-5 border border-green-200/50 dark:border-green-700/50">
                            <div className="flex items-center justify-between">
                                <div><p className="text-sm text-gray-600 dark:text-gray-400 mb-1">وحدات منشورة</p><p className="text-3xl font-bold text-gray-900 dark:text-white">{lessons.filter((l: any) => l.is_published).length}</p></div>
            ...
            {isLoading ? (
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">{searchQuery ? "لا توجد نتائج" : "لا توجد وحدات"}</h3>
                    <p className="text-gray-500 dark:text-gray-400 max-w-sm mb-6">{searchQuery ? `لم يتم العثور على وحدات تطابق "${searchQuery}"` : "لم تقم بإضافة أي وحدات بعد."}</p>
                    {!searchQuery && <button onClick={() => { setCurrentLesson({ is_published: false }); setIsModalOpen(true); }} className="flex items-center gap-2 px-6 py-3 rounded-xl bg-purple-500 hover:bg-purple-600 text-white font-semibold transition-colors"><Plus className="h-5 w-5" /><span>إضافة وحدة جديدة</span></button>}
                </motion.div>
            ) : (
                <>
                            ...
                            >
                                <div className="p-4">
                                    ...
                                    <div className="flex items-center justify-between pt-2 border-t border-gray-100 dark:border-gray-800">
                                        <Link href={`/admin/lessons?unit=${lesson.id}`} className="flex items-center gap-1 text-xs text-purple-600 dark:text-purple-400 hover:underline font-medium"><MessageSquare className="h-3 w-3" /><span>الدروس</span></Link>
