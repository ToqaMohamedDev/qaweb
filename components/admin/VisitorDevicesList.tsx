...
function VisitorDeviceCard({
    ...
    return (
        <motion.div
            ...
            className={`relative bg-white dark:bg-[#1a1a1a] rounded-xl border border-gray-200 dark:border-[#2a2a2a] p-4 hover:shadow-lg transition-shadow`}
        >
            <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                        ...
                        <span className="text-xs bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded-full text-gray-600 dark:text-gray-400 font-mono">
                    ...
                    {(device.page_url || device.referrer) && (
                        <div className="mt-2 pt-2 border-t border-gray-100 dark:border-gray-800 space-y-1">
                            {device.page_url && (
                                <p className="text-xs text-gray-500 truncate" title={device.page_url}>
                                    <span className="font-medium">آخر صفحة:</span> {new URL(device.page_url).pathname}
                                </p>
                            )}
                            {device.referrer && (
                                <p className="text-xs text-gray-500 truncate" title={device.referrer}>
                                    <span className="font-medium">المصدر:</span> {new URL(device.referrer).hostname}
                                </p>
                            )}
                        </div>
                    )}
                    ...
                    <div className="flex items-center gap-4 mt-2 text-xs">
                        ...
                        <span className="text-gray-400">
                            أول ظهور: <span className="font-semibold text-gray-600 dark:text-gray-300">{new Date(device.first_seen_at).toLocaleDateString('ar-EG')}</span>
                        </span>
                    </div>
                </div>
